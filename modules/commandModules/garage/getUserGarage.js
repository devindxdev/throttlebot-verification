...
module.exports = async (userId, guildProfile) => {
    try {
        ...
        // Check if the user has any verified vehicles
        if (!combinedVehicles || combinedVehicles.length === 0) {
            return null; // No vehicles found
        }

        return combinedVehicles; // Return the combined garage data
    } catch (error) {
